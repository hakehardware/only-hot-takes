...
class DB:
    ...
    def store_token(self, token):
        ...
        with self.connect() as conn:
            c = conn.cursor()
            c.execute("DELETE FROM oauth_tokens")  # Only store one token at a time
            c.execute("INSERT INTO oauth_tokens (token, expires_at, refresh_token) VALUES (?, ?, ?)",
    ...
    def get_refresh_token(self):
        ...
        with self.connect() as conn:
            c = conn.cursor()
            c.execute("SELECT refresh_token FROM oauth_tokens ORDER BY expires_at DESC LIMIT 1")
            row = c.fetchone()
